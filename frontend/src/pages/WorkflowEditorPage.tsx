...
import { CheckCircle, Download, Expand, Minimize2, Play, Plus, Save, Upload } from 'lucide-react';
...
const initialNodes: Node[] = [
  { id: 'start', type: 'input', position: { x: 80, y: 180 }, data: { label: '开始：读取项目目标', node_type: 'start', config: { auto_mode: true } } },
  { id: 'ai-task', position: { x: 390, y: 180 }, data: { label: 'AI 任务：分析与计划', node_type: 'ai_task', config: { prompt: '请总结项目目标并输出下一步执行计划' } } },
  { id: 'command', position: { x: 720, y: 180 }, data: { label: '命令：测试/构建', node_type: 'command', config: { command: 'npm run build', cwd: '.' } } },
  { id: 'end', type: 'output', position: { x: 1040, y: 180 }, data: { label: '结束：交付总结', node_type: 'end', config: {} } }
];
const initialEdges = [
...
const nodeCatalog: Array<{ type: string; label: string; group: string; config?: Record<string, unknown> }> = [
  ...
  { type: 'ui_design', label: 'UI 设计节点', group: '研发', config: { style: 'cursor-like light/dark professional' } },
      ...
      <div className="workflow-grid">
        <Card className="node-palette">
          ...
          <div className="node-catalog">
            {nodeCatalog.map(node => <button key={node.type} onClick={() => addNode(node.type)}><span><strong>{node.label}</strong><small>{node.group}</small></span><em>{node.type}</em></button>)}
          </div>
          <h2>工作流</h2>
        ...
        <Card className="properties">
          ...
          </> : <div className="empty">选择节点后编辑属性。可先点击“放大画布”专注拖拽。</div>}
